...
function bash_prompt() {
    ...
    specialpart="${USER}@local"
    # Red prompt if production system
    if [ $sysmode = 'PROD' ]; then
        specialpart="$inverse$red$specialpart$uninverse"
    fi
    if [ $sysmode = 'Test' ]; then
        specialpart="$magenta$specialpart"
    fi
    if [ $sysmode = 'Stage' ]; then
        specialpart="$green$specialpart"
    fi
    if [ $sysmode = 'Dev' ]; then
        specialpart="$cyan$specialpart"
    fi
    ...
    if [ "$branch" = "master" ]; then
      branch="$inverse$red$branch$uninverse$green";
    fi
    prompt="${debian_chroot:+($debian_chroot)}$yellow${specialpart}${kube} ${branch:+$green[$branch] }${lightning}$blue${promptpath}\$$black$reset "
    echo -n "$prompt"
}
